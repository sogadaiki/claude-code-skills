...
- `~/.claude/knowledge/document-design.md` — デザイン原則・レイアウトパターン
- `~/.claude/knowledge/slide-design-prompts.md` — スライドデザインのプロンプト集

## スライドNG集（AI臭さ排除）

**全プレゼンで必ず守る。** `design-standards.md` Section 3 の全環境共通NG + 以下のスライド固有NG。

| NG | なぜNG | 代替 |
|----|--------|------|
| **全スライド同じレイアウト** | AIが同じテンプレートを繰り返す典型 | スライドごとにレイアウトを変える（カラム、ボックス、引用、フルブリード） |
| **SaaSデフォルト色（#2196F3, #06b6d4）をそのまま使用** | base-styles.cssの初期値を変更せず使うとAIテンプレ感 | 必ずカスタムカラーに変更。18パレット例から選ぶか独自定義 |
| **箇条書きだけのスライドが3枚以上連続** | プレゼンとして退屈 | 引用、コールアウト、図表、カラムを混ぜる |
| **装飾線（::before/::after）の多用** | 過装飾 | border-leftか背景色で十分 |
| **Font Awesomeアイコンの全スライド配置** | アイコン依存 | 本当に意味がある箇所だけ |
...
| Pass | 内容 | 対応Step |
|------|------|----------|
| **Pass 1: 素材収集** | コンテンツ読み込み、ナレッジ参照（design-standards.md等）、ベンチマークスライド参考 | Step 1 |
| **Pass 2: 構成設計 + 自己批判** | スライド構成・配色・レイアウト方針を設計。設計後に「1スライド1メッセージか？」「同じレイアウトが3枚連続していないか？」「NG集に抵触していないか？」を自己批判 | Step 2-3 |
| **Pass 3: 生成** | HTML + CSS実装。scaffold生成→CSS カスタマイズ→コンテンツ記入 | Step 4 |
| **Pass 4: 品質ゲート** | overflow check + 全スライドスクリーンショット目視。以下をチェック | Step 5-6 |
| **Pass 5: 修正** | FAIL項目のみ修正→再スクリーンショットで検証。2回FAILで該当スライド再設計 | Step 6後半 |

### Pass 4 品質ゲート項目（スライド固有）

| # | チェック項目 | FAIL基準 |
|---|------------|---------|
| 1 | レイアウト多様性 | 同じレイアウトが3枚以上連続 |
| 2 | 文字量 | 1スライドに箇条書き8項目超 or 段落テキスト |
| 3 | 色継承 | コンテナ内テキストが背景と同化して読めない |
| 4 | AI臭さ | NG集（SaaSデフォルト色/装飾線多用/アイコン全配置）に抵触 |
| 5 | overflow | check-overflow.jsでエラー検出 |
| 6 | 1メッセージ原則 | 1スライドに複数の主張が混在 |
