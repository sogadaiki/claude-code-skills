...
> 40個あるけど、まずはここから。これだけで開発体験が変わります。

### 1. セッション管理トリオ — `morning` / `resume` / `finish-session`

毎日これで始まり、これで終わる。AIが「昨日どこまでやったっけ？」を覚えてくれる。
セッションを跨いでも文脈が途切れない。人間がメモを取る必要がない。

**なぜセッションを区切るのか？** — Claude は200kトークンのコンテキストを持つが、会話が長くなると自動圧縮（compact）が走り、**冒頭で設定した方針・トーン・設計意図が要約で丸められて薄れていく**。LLM の注意機構は直近のトークンに重みが偏るため、最初の指示ほど忘れられやすい。

だから **1セッション=1タスクで区切る**。文脈をコンテキストウィンドウ内に溜め込まず、`/finish-session` で外部ファイル（restart.md）に保存 → 次回 `/resume` で必要最小限だけ再ロードする。これがcompactを起こさず、冒頭の指示を最後まで維持する運用パターン。

```
/morning          → 朝イチで全体把握。Issue横断、関係温度マップ、今日やること提案
/resume           → 中断からの即復帰。restart.mdとgit statusだけで5秒で再開
/finish-session   → 「今日はここまで」で進捗を自動記録。次回/resumeで完全再開
```
...
いきなりコードを書かない。設計図を完璧にしてから実装に入る。
Devil's Advocate（全否定検証）を挟んで、計画段階で穴を潰す。

```
/plan ユーザー認証機能の追加
→ 設計 → Devil's Advocate → 修正 → 確定 → Issue作成 → 実装開始
```

### 4. エージェントチーム並列分析 — `business-review` / `map-project`

1つの課題に対して、複数の専門エージェントが同時にレビューする。
例えば `business-review` は戦略・財務・マーケ・法務の4視点が並列で分析して統合レポートを出す。

人間1人 vs. 専門家チーム。この差がスキル1つで手に入る。

### 5. ミスを二度としない — `learn-mistake`

指摘されたミスからルールを自動策定 → CLAUDE.mdに書き込む。
次のセッションから、同じミスは物理的に起きなくなる。AIが勝手に成長する仕組み。

```
/learn-mistake Stripe金額をセント単位で送ってしまった
→ ルール策定 → CLAUDE.md更新 → 次回から自動チェック
```
